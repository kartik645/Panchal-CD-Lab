...
char terminal[20],nonterminal[20],alpha[10];
char x,epsilon='^';
...
int main()
{
    ...
    cin>>f;
    cout<<"Enter the non_terminal symbols for your production:";
    for(k=0;k<f;k++)
    {
        cin>>nonterminal[k];
    }
    ...
    for(k=0;k<=n-1;k++)
    {
        cout<<"Enter the"<<k+1<<"productions:";
        cin>>prod[k].l;
        cout<<"->";
        cin>>prod[k].r;
    ...
    for(int m=0;m<f;m++)
    {
        ...
        for(int j=0;j<n;j++)
        {
            if((prod[j].l==x)&&(prod[i].r[0]==prod[j].l))
                flag=1;
        }
        for(int i=0;i<n;i++)
        {
